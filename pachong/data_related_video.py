...
metadata = pd.read_csv('jiangping_metadata2024_06_26.csv')
metadata = metadata.drop_duplicates(subset=['URL'])
date_execution = '2024/06/28'
...
def extract_data(card, meta_url, meta_title, meta_author, meta_playbacks, meta_dm, meta_full_title, date_execution):
    try:
...
for index, row in metadata.iterrows():
    try:
        ...
        if next_play_card:
            first_video_data = extract_data(next_play_card, meta_url, meta_title, meta_author, meta_playbacks, meta_dm,meta_full_title, date_execution)
        ...
        if rec_list_section:
            ...
            for card in rec_list_cards:
                rec_video_data = extract_data(card, meta_url, meta_title, meta_author, meta_playbacks, meta_dm, meta_full_title, date_execution)
                all_video_data.append(rec_video_data)

        number_of_times += 1  # Increment the counter
        print(f"Processed {number_of_times} items")

        if number_of_times % 100 == 10:
                    temp_df = pd.DataFrame(all_video_data)
                    temp_df.to_csv(f'related_video_data_2_temp_{number_of_times}.csv', index=False)
...
df.to_csv('related_video_data_2.csv', index=False)
...
print("Data has been written to related_video_data_2.csv")
